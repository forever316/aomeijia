<div class="form-group" id="typeDiv">
    <label class="col-sm-1 control-label">通用属性</label>
            ...
            @if(!empty($detailData->fieldArray))
                @foreach($detailData->fieldArray as $key=>$item)
                    <div id="ziduan_{{$key}}" style="overflow:hidden;width: 100%;">
                        <input class="form-control" name="field[{{$key}}]" value="{{$item}}" placeholder="属性名称" style="width: 30%;float: left;margin-right: 20px;" />
